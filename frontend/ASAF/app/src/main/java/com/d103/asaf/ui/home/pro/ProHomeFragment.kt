...
class ProHomeFragment : BaseFragment<FragmentProHomeBinding>(FragmentProHomeBinding::bind, R.layout.fragment_pro_home) {
    ...
    fun initToggleButton(){
        ...
        // 반 정보 받아 오기
        sharedViewModel.classInfoList

        // 반 갯수 별로 toggleButton 변경
        when(sharedViewModel.classInfoList.value?.size){
            1 -> {
                ...
                binding.fragmentProHomeCustomtoggleButton.seatText.setText("${sharedViewModel.classInfoList.value?.get(0)?.classCode} 반")
            }
            2 -> {
                binding.fragmentProHomeCustomtoggleButton.moneyText.visibility = View.GONE
                binding.fragmentProHomeCustomtoggleButton.seatText.setText("${sharedViewModel.classInfoList.value?.get(0)?.classCode} 반")
                binding.fragmentProHomeCustomtoggleButton.lockerText.setText("${sharedViewModel.classInfoList.value?.get(1)?.classCode} 반")

            }
            3 -> {
                binding.fragmentProHomeCustomtoggleButton.seatText.setText("${sharedViewModel.classInfoList.value?.get(0)?.classCode} 반")
                binding.fragmentProHomeCustomtoggleButton.lockerText.setText("${sharedViewModel.classInfoList.value?.get(1)?.classCode} 반")
                binding.fragmentProHomeCustomtoggleButton.moneyText.setText("${sharedViewModel.classInfoList.value?.get(2)?.classCode} 반")
    ...
    fun initstudentList(classInfo : Classinfo){
        ...
        proHomeFragmentViewModel.getStudentsInfo(classInfo.classNum)
        var studentList = proHomeFragmentViewModel.studentInfoList.value!!
        var attendedCount  = 0f
        for(student : Member in studentList){
            if(student.attended == "출석"){
                attendedCount++;
            }

        }
        attendedPercent = (attendedCount/studentList.size.toFloat() * 100).toInt().toFloat()
